...
class EncoderStyledLayer(nn.Module):
    """Encoder layer module.
    ...
    def __init__(
        ...
        """Construct an EncoderLayer object."""
        super(EncoderStyledLayer, self).__init__()
        self.self_attn = self_attn
        self.feed_forward = feed_forward
        self.ff_scale = 1.0
        self.conv_module = conv_module
        self.macaron_style = macaron_style
        self.norm_ff = StyleAdaptiveLayerNorm(size, style_dim)  # for the FNN module
        self.norm_mha = StyleAdaptiveLayerNorm(size, style_dim)  # for the MHA module
        if self.macaron_style:
            self.feed_forward_macaron = copy.deepcopy(feed_forward)
            self.ff_scale = 0.5
            # for another FNN module in macaron style
            self.norm_ff_macaron = StyleAdaptiveLayerNorm(size, style_dim)
        if self.conv_module is not None:
            self.norm_conv = StyleAdaptiveLayerNorm(size, style_dim)  # for the CNN module
            self.norm_final = StyleAdaptiveLayerNorm(size, style_dim)  # for the final output of the block
        self.dropout = nn.Dropout(dropout_rate)
        self.size = size
        self.normalize_before = normalize_before
        self.concat_after = concat_after
    ...
    def forward(self, x_input, mask, style, cache=None):
        """Compute encoded features.
        ...
        if self.macaron_style:
            residual = x
            if self.normalize_before:
                x = self.norm_ff_macaron(x, style)
            x = residual + self.ff_scale * self.dropout(self.feed_forward_macaron(x))
            if not self.normalize_before:
                x = self.norm_ff_macaron(x, style)
