...
from espnet.nets.pytorch_backend.transformer.encoder_layer import EncoderLayer, EncoderStyledLayer, StyleAdaptiveLayerNorm
...
class Encoder(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.normalize_before:
            if is_styled:
                self.after_norm = StyleAdaptiveLayerNorm(attention_dim, style_dim)
            else:
    ...
    def forward(self, xs, masks, extract_resnet_feats=False):
        ...
        if is_styled:
            for encoder in self.encoders:
                xs, masks = encoder(xs, masks, style)
    ...
    def forward_after_frontend(self, xs, masks, style=None):
        """Encode input sequence.
...
class EncoderConformerFusion(torch.nn.Module):
    """Transformer encoder module.
    ...
    def __init__(
        ...
        """Construct an Encoder object."""
        super(EncoderConformerFusion, self).__init__()
        self._register_load_state_dict_pre_hook(_pre_hook)
