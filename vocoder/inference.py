...
def init_worker(queue, arguments):
    ...
    if a.code_file is not None:
        ...
        for code_path in file_list:
            ...
            #lrs2
            spk_emb_path = code_path.replace(f"{a.code_file}/", "../datasets/lrs2/spk_emb/")[:-4]+".npy"
            # #lrs3
            l = code_path.split('/')[-3:]

            # Join the last 3 parts to form a new path
            emb_path = os.path.join(*l).replace('.txt', '.npy')

            spk_emb_path =  "/cm/archive/khanhpl2/LRS3-TED/dataset4ISU/data/spk_emb/" + emb_path


            code_str = open(code_path).readline().strip()
            code = parse_code(code_str)
            mel = np.load(mel_path).squeeze(0).transpose(1,0)
...
def main():
    ...
    print(f"Pure inference time: {total_inference_time:.2f} seconds")
    print(f"Average time per sample: {total_inference_time/num_samples:.2f} seconds")
    print(f"Throughput: {num_samples/total_time:.2f} samples/second")
