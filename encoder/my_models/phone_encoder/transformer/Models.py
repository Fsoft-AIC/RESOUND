...
from .Layers import FFTBlock, StyleFFTBlock
...
class Lip_PositionalEmbedding(nn.Module):
  
    def __init__(self, cfg):
        super(Lip_PositionalEmbedding, self).__init__()

        n_position = cfg.max_seq_len + 1
        d_word_vec = 512
        d_model = cfg.encoder_hidden
        ...
        self.fc_out = nn.Linear(d_model, 512)
...
class Lip_Encoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        #     requires_grad=False,
        # )
        self.fc_out = nn.Linear(self.d_model, 512)
    ...
    def forward(self, src_seq, mask, return_attns=False):
        ...
        return enc_output
class PhoneEncoder(torch.nn.Module):
    """ Encoder """
    ...
    def forward(self, src_seq, mask, return_attns=False):
        ...
        if not self.training and src_seq.shape[1] > hp.max_seq_len:
            enc_output = self.src_word_emb(src_seq) + get_sinusoid_encoding_table(
                src_seq.shape[1], encoder_hidden
            )[: src_seq.shape[1], :].unsqueeze(0).expand(batch_size, -1, -1).to(
                src_seq.device
            )
        else:
...
class StylePhoneEncoder(torch.nn.Module):
    """ Encoder """
    ...
    def __init__(
        ...
        super(StylePhoneEncoder, self).__init__()
        self.training = training
        n_position = len_max_seq + 1
        ...
        self.layer_stack = nn.ModuleList(
            [
                StyleFFTBlock(d_model, d_inner, n_head, d_k, d_v, dropout=dropout, style_dim=style_dim)
                for _ in range(n_layers)
            ]
        )
    def forward(self, src_seq, mask, style_code, return_attns=False):
        ...
        for enc_layer in self.layer_stack:
            ...
            enc_output, enc_slf_attn = enc_layer(
                enc_output, style_code=style_code, mask=mask, slf_attn_mask=slf_attn_mask
            )
            if return_attns:
                enc_slf_attn_list += [enc_slf_attn]
