...
class CoarsedMelGenerators(nn.Module):
    def __init__(self, generator_cfg: GeneratorConfig, cfg: AVHubertAsrConfig):
        ...
        self.mel_conv = nn.Sequential(
            nn.Conv1d(in_channels=cfg.encoder_hidden,out_channels=cfg.encoder_hidden,kernel_size=3,stride=1,padding=1),
    ...
    def forward(self, f0_tokens, energy_tokens, filter_input, padding_mask, tbc=True, spk_emb=None, hidden_query=None):
        source_input = self.source_generator(f0_tokens, energy_tokens, mask=padding_mask, hidden_query=hidden_query)
        filter_input = self.filter_generator(filter_input, mask=padding_mask, hidden_query=hidden_query)

        assert source_input.shape == filter_input.shape

        coarse_mel = source_input + filter_input

        if tbc:
            coarse_mel = coarse_mel.transpose(0,1)  
        
        if self.proj_out:
            x = self.proj_out(coarse_mel) # for calculate CE loss with Hubert
            # B x T x 204
